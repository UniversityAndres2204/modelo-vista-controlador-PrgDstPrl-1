...
import {
  ...
  eliminarPropietario,
  crearPropietario,
} from "@/lib/models/propietarioModel";
...
export default function PropietariosList({userId, propietarios}: { userId: string, propietarios: Propietario[] }) {
  ...
  return (
    <div className="w-full max-w-5xl">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-zinc-400 dark:text-zinc-500 mt-1">
            {lista.length} propietarios registrados
          </p>
        </div>
      ...
      {/* Tabla header */}
      <div className="grid grid-cols-[1fr_1.2fr_1fr_1.8fr_auto] gap-4 px-4 pb-2 mb-1">
        {["Identificación", "Tipo", "Nombre", "Dirección", "Acciones"].map(
          (col) => (
            <span
              key={col}
              className="text-xs font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest"
            >
              {col}
            </span>
          )
      ...
      <ul className="flex flex-col gap-2">
        {lista.map((propietario) => (
          <li
            ...
            className="grid grid-cols-[1fr_1.2fr_1fr_1.8fr_auto] gap-4 items-center
          ...
          >
            ...
            {editando === propietario.identificacion ? (
              <select
                ...
                onChange={(e) =>
                  setTemp({
                    ...temp,
                    tipo: e.target.value as TipoPropietario,
                  })
                }
              >
              ...
              <span
                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold w-fit ${tipoBadgeColor[propietario.tipo]}`}
              >
            ...
            {editando === propietario.identificacion ? (
              <input
                ...
                value={temp.nombre ?? ""}
                onChange={(e) =>
                  setTemp({...temp, nombre: e.target.value})
                }
              />
            ) : (
            ...
            {editando === propietario.identificacion ? (
              <input
                ...
                onChange={(e) =>
                  setTemp({...temp, direccion: e.target.value})
                }
              />
            ) : (
